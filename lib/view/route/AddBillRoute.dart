...
class AddBillRoute extends StatefulWidget {
  final int billProperty;

  AddBillRoute(this.billProperty);

  @override
  State<StatefulWidget> createState() {
    return AddBillRouteState(this.billProperty);
  }
}

class AddBillRouteState extends State<AddBillRoute>
  ...
  AddBillRouteState(this.billProperty);

  @override
  void initState() {
    super.initState();
    presenter = AddBillRoutePresenter(this);
    billDateButtonTitle = presenter.getCurrentFormatTime(DateTime.now());

    if (billProperty > 0) {
      categoryContent = "收入分类";
      accountContent = "收入账户";
    } else {
      categoryContent = "支出分类";
      accountContent = "支出账户";
    }
  ...
  Future checkPermission() async {
    Future<int> camera = presenter.checkPermission(await PermissionHandler().checkPermissionStatus(PermissionGroup.camera));
    Future<int> microphone = presenter.checkPermission(await PermissionHandler().checkPermissionStatus(PermissionGroup.microphone));
    camera.then((value) {
      setState(() {
